...
function graph(filepath) {
    ...
    return new Promise((resolve) => {
        ...
        d3.json(filepath).then(data => {
            ...
            let margin = { top: 0, right: 30, bottom: 40, left: 20 },
                width = 850, height = 300, duration = 300;
            ...
            const initialScale = 0.2;  // Set this to a value less than 1 to zoom out
            const initialTranslateX = width / 2;  // Center horizontally
            const initialTranslateY = height / 2; // Center vertically
            ...
            label = d3.select('.labels')
                ...
                .join(
                    enter => enter.append('text')
                        ...
                        .text(d => { if (d.degree > 100) { return d.name } else { return '' } })
...
tooltip
    ...
    .attr('pointer-events', 'none')
    .style('opacity', 0.7) // lowered opacity so nodes/links behind can be seen
    .style('color', "#fff")
    ...
    tooltip.style("left", (event.pageX + 10) + "px")
           .style("top", (event.pageY + 10) + "px");
            ...
            node.on('click', function (event, d) {
                var url = "https://everynoise.com/engenremap-" + d.id + ".html"
